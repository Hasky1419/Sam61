...
router.get('/iframe', async (req, res) => {
  try {
    ...
    if (vodPath) {
      ...
      videoUrl = `http://${wowzaHost}:1935/${userLogin}/_definst_/mp4:${vodPath}/playlist.m3u8`;
      title = `VOD: ${vodPath}`;
      isLive = false;
    } else if (stream) {
      // Stream ao vivo
      ...
      try {
        ...
        if (playlistRows.length > 0) {
          ...
          const [videoRows] = await db.execute(
            'SELECT v.url, v.nome FROM videos v WHERE v.playlist_id = ? ORDER BY v.id LIMIT 1',
            [playlist]
          );
          
          if (videoRows.length > 0) {
            videoUrl = `/content/${videoRows[0].url}`;
      ...
      try {
        const [videoRows] = await db.execute(
          'SELECT url, nome FROM videos WHERE id = ?',
          [video]
        );
